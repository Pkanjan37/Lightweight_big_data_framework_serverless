...
from .storage_utils import create_status_key, create_output_key,create_state_output_key
...
class Storage(object):
    ...
    def get_storage_config_wrapped(self):
        return self.storage_config['backend_config']
    ...
    def predefine_put_data(self,call_id,timeStmp,func):
        path = func+"-"+str(timeStmp)+"/"+call_id+"/input.json"
        return path
    ...
    def construct_function_binary(self,func,path):
        fullZip = path+"\\"+func+".zip"
        func_desc = open(fullZip, 'rb')
        return func_desc
    ...
    def put_cancelled(self, callset_id, call_id, cancelled_data):
        ...
        :param func: cancelled_data
        :return: None
        """
        key = create_cancel_key(self.prefix, callset_id,
                                call_id)
        return self.backend_handler.put_object(key, cancelled_data)
    ...
    def get_state_output(self,path):
        ...
        try:
            ...
            raise StorageOutputNotFoundError(e)

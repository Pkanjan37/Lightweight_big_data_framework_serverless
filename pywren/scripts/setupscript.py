...
from pywren.scripts import lightweightcli
...
def create_unique_bucket_name():
    bucket_name = "{}-lightweight-{}".format(get_username().lower(),
...
def interactive_setup(ctx, dryrun, suffix):
    ...
    click.echo("This is the interactive setup script")
    #first we will try and make sure AWS is set up

    account_id = ctx.invoke(lightweightcli.get_aws_account_id, False)
    click.echo("Your AWS configuration appears to be set up, and your account ID is {}".format(
        account_id))

    click.echo("This interactive script will set up your initial  configuration.")
    click.echo("If this is the first time you are using, we recommand the defaults configuration")
    ...
    s3_bucket = click_validate_prompt(
        "Lightweight requires an s3 bucket to store intermediate data. " + \
    ...
    output_bucket = click_validate_prompt(
        ...
        validate_func=check_valid_bucket_name)
    create_bucket = False
    if not check_bucket_exists(output_bucket):
        create_bucket = click.confirm(
            "Bucket does not currently exist, would you like to create it?", default=True)
